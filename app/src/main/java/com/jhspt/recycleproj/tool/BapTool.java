...
public class BapTool {
    ...
    public static restoreBapDateClass restoreBapData(Context mContext, int year, int month, int day) {
        ...
        if (mData.Morning == null && mData.Lunch == null && mData.Dinner == null) {
    ...
    public static todayBapData getTodayBap(Context mContext) {
        ...
        if (!mData.isBlankDay) {
             ...
             */
            if(0 <= hour && hour < 8) {
                mReturnData.title="오늘의 아침";
                mReturnData.info = (!MealLibrary.isMealCheck(mData.Morning) ? mContext.getString(R.string.no_data_morning) : mData.Morning);
            }
